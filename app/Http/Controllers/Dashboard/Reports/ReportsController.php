    ...
    protected function sales(Request $request)
    {
        if (request()->ajax()) {
            ...
            if($date) {
                ...
                $carbonDate = \Carbon\Carbon::parse($date)->timezone('Asia/Riyadh')->timezone('Asia/Riyadh');
                $formattedDate = $carbonDate->format('Y-m-d H:i:s');
                $order = $order->where('created_at','>=', $formattedDate);
            }
            if($date2){
         
                $carbonDate2 = \Carbon\Carbon::parse($date2)->timezone('Asia/Riyadh')->timezone('Asia/Riyadh');
                ...
                ->addColumn('late', function ($row) {
                    ...
                    if ($service_ids != []){
                        ...
                        foreach ($visits as $visit){
                            ...
                            $duration += Carbon::parse($end_time)->timezone('Asia/Riyadh')->diffInMinutes(Carbon::parse($start_time)->timezone('Asia/Riyadh'));
                        }
